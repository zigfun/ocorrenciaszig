...
function fmt(d) { try { if (!d) return ""; const s = d.includes("/") ? d.split("/").reverse().join("-") : d; return new Date(s + "T12:00:00").toLocaleDateString("pt-BR"); } catch { return d; } }
...
.modal{background:var(--sf);border:1px solid var(--bd);border-radius:16px;width:100%;max-width:580px;max-height:92vh;overflow-y:auto;padding:26px;}
...
.det-tags{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:16px;}
...
.del-ic{width:26px;height:26px;border-radius:6px;border:1px solid var(--bd);background:transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:12px;color:var(--mu);transition:all .12s;}
...
export default function App() {
  ...
  const [data,    setData]    = useState({ incidents: [], users: [] });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({ incidents: d.incidents || [], users: d.users || [] });
      setSync("ok");
    } catch { setSync("err"); }
  }, [apiKey, binId]);
  ...
  const handleSetup = async () => {
    ...
    try {
      const init = {
        incidents: [],
        users: [{ id: "u_" + Date.now(), name: sName.trim(), email: sEmail.toLowerCase().trim(), passwordHash: simpleHash(sPass.trim()), isAdmin: true }]
      };
      ...
      setApiKey(sKey.trim()); setBinId(id); setSession(sess); setData(init);
    } catch (e) { setSErr("Erro: " + (e.message || "verifique a API Key")); }
    setSLoad(false);
  };

  const handleLogin = () => {
    const u = data.users.find(u => u.email === lEmail.toLowerCase().trim() && u.passwordHash === simpleHash(lPass));
  ...
  const exportXlsx = () => {
    const rows = filtered.map(i => ({
      "Cliente": i.client, "Data": i.date, "AM": i.am,
      "Time/Coord": i.coord, "Área": i.category,
      ...
      <div className="auth">
        <div className="auth-card">
          <div className="auth-brand">
            <div className="auth-ic">📋</div>
            <div><div className="auth-name">Ocorrências ZIG</div><div className="auth-sub">Configuração inicial</div></div>
          </div>
          <div className="auth-title">Criar banco de dados</div>
          <div className="auth-desc">Configure uma vez. Depois compartilhe o link com o time — cada pessoa faz seu login.</div>
          <div className="steps">
    ...
    <>
      ...
      <div className="auth">
        <div className="auth-card">
          <div className="auth-brand">
            <div className="auth-ic">📋</div>
            <div><div className="auth-name">Ocorrências ZIG</div><div className="auth-sub">Central de registros</div></div>
          </div>
        ...
        <aside className="sb">
          <div className="sb-top">
            <div className="sb-brand">
              ...
              <div><div className="sb-name">Ocorrências</div><div className="sb-sub">ZIG Funtech</div></div>
            </div>
          </div>
          ...
          <div className="nav-sec">
            ...
            <div className={`ni ${view === "kanban" ? "on" : ""}`} onClick={() => setView("kanban")}>
              ...
              <span className="ni-badge">{incidents.filter(i => (i.status || "andamento") === "andamento").length}</span>
            </div>
            {session.isAdmin && (
              <div className={`ni ${view === "users" ? "on" : ""}`} onClick={() => setView("users")}>
                <span className="ni-ic">👥</span> Usuários <span className="ni-badge">{data.users.length}</span>
              </div>
            )}
          </div>
          {allAreas.length > 0 && (
        ...
        <main className="main">
          <div className="topbar">
            <div className="page-title">
              {view === "kanban" ? <>Kanban de <span>Status</span></> : view === "users" ? <><span>Usuários</span></> : <>Dashboard de <span>Ocorrências</span></>}
            </div>
            ...
            <div className="content">
              ...
              {filtered.length === 0 ? (
                <div className="empty">
                  ...
                  <div className="empty-t">{incidents.length === 0 ? "Nenhuma ocorrência ainda.\nAs respostas do Forms aparecerão aqui automaticamente." : "Nenhum resultado para os filtros aplicados."}</div>
                </div>
              ) : (
                <>
                  {(filterArea ? [filterArea] : allAreas).map(area => {
                    ...
                    return (
                      <div key={area}>
                        ...
                        <div className="grid">
                          {aIncs.map((inc, i) => <IncCard key={i} inc={inc} onClick={() => setActive(inc)} />)}
                        </div>
                      </div>
                    );
                  })}
          ...
          {view === "kanban" && (
            <div className="kanban">
              {[
                ...
                const colIncs = filtered.filter(i => (i.status || "andamento") === col.key);
                return (
...
function DetailModal({ inc, onClose }) {
  const ac = aColor(inc.category);
  const status = inc.status || "andamento";
  const extras = Object.entries(inc).filter(([k]) => !["id","client","date","am","coord","category","status","desc","attachments","custom","registeredBy"].includes(k) && inc[k]);
  return (
    <div className="ov" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal lg">
        ...
        {extras.length > 0 && (
          <div className="det-sec">
            <div className="det-sec-lbl">Informações adicionais</div>
            <div className="det-grid">
              {extras.map(([k, v]) => (
                <div key={k} className="det-item">
                  <div className="det-item-lbl">{k}</div>
                  <div className="det-item-val">{String(v)}</div>
                </div>
              ))}
            </div>
          </div>
